...
export default function Seguradoras() {
  ...
  function abrirModalSeguradora(id: number) {
    setModal({
      ...
      message: "Deseja excluir esta seguradora? Esta ação não poderá ser desfeita.",
      onConfirm: async () => {
        try {
          setDeletingSegId(id)
  ...
  function abrirModalProduto(idProduto: number, idSeguradora: number) {
    setModal({
      ...
      message: "Deseja excluir este produto? Esta ação não poderá ser desfeita.",
      onConfirm: async () => {
        try {
          ...
          setSeguradoras((prev) =>
            prev.map((s) =>
              s.idSeguradora === idSeguradora
                ? {
                    ...s,
                    produtos: s.produtos.filter((p) => p.idProduto !== idProduto),
                  }
                : s
            )
          )
  ...
  return (
    <MainLayout title="Seguradoras">
          ...
          <form className="mt-4 space-y-3" onSubmit={handleCriarProduto}>
            <div>
              ...
              <select
                value={seguradoraId}
                onChange={(e) =>
                  setSeguradoraId(Number(e.target.value) || "")
                }
      ...
      <div className="mt-8 bg-white rounded-2xl shadow p-6 border border-gray-100">
        <h2 className="text-lg font-semibold text-brand-dark">Seguradoras</h2>
        {loading ? (
                    ...
                    {editId === s.idSeguradora ? (
                      <>
                        <button
                          ...
                          disabled={updatingSegId === s.idSeguradora}
                        >
                          {updatingSegId === s.idSeguradora
                            ? "Salvando..."
                            : "💾 Salvar"}
                        </button>
                        <button
                      ...
                      <>
                        ...
                        <button
                          onClick={() => abrirModalSeguradora(s.idSeguradora)}
                  ...
                  <ul className="mt-3 text-sm text-gray-600 space-y-2">
                    {s.produtos.map((p) => {
                      ...
                      return (
                        <li
                          key={p.idProduto}
                          className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 bg-white rounded-md px-3 py-2"
                        >
                          {isEditing ? (
                            ...
                            {isEditing ? (
                              <>
                                <button
                                  ...
                                  disabled={updatingProdId === p.idProduto}
                                >
                              ...
                              <>
                                ...
                                <button
                                  onClick={() =>
                                    abrirModalProduto(p.idProduto, s.idSeguradora)
                                  }
